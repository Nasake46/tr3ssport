...
const verifyUserEmail = async (email: string): Promise<{ exists: boolean; userData?: any }> => {
  try {
    ...
    const usersQuery = query(collection(firestore, 'users'), where('email', '==', email.toLowerCase().trim()));
...
const verifyAllInvitedEmails = async (
  emails: string[],
): Promise<{ valid: string[]; invalid: string[]; userData: any[] }> => {
  const results = await Promise.all(emails.map((email) => verifyUserEmail(email)));
  const valid: string[] = [];
  ...
  try {
    if (formData.invitedEmails && formData.invitedEmails.length > 0) {
      ...
      if (emailVerification.invalid.length > 0) {
        throw new Error(
          `Emails non valides (utilisateurs non inscrits): ${emailVerification.invalid.join(', ')}`,
        );
      }
    ...
    if (formData.invitedEmails && formData.invitedEmails.length > 0) {
      ...
      for (const u of emailVerification.userData) {
        const invitationData = {
          ...
          invitedUserName: `${u.firstName || ''} ${u.lastName || ''}`.trim() || u.email,
...
const getAllCoaches = async (): Promise<Coach[]> => {
  try {
    ...
    const coachesQuery = query(collection(firestore, 'users'), where('role', '==', 'coach'));
...
export default function CreateAppointmentScreen() {
  ...
  // ---- FIX Android DateTimePicker: séquence date -> time (pas de mode "datetime" sur Android)
  ...
  const onPickerChange = (event: DateTimePickerEvent, selected?: Date) => {
    if (Platform.OS === 'android') {
      if (pickerMode === 'date') {
        ...
        // Applique l'heure, garde la date déjà choisie
        if (event.type === 'set' && selected) {
          const current = new Date(formData.date);
          current.setHours(selected.getHours(), selected.getMinutes(), 0, 0);
          console.log('📅 CRÉATION RDV - Date sélectionnée:', current.toISOString());
          setFormData((s) => ({ ...s, date: current }));
        }
        setShowPicker(false);
      }
    } else {
      // iOS: un seul picker en mode 'datetime'
      if (selected) {
        console.log('📅 CRÉATION RDV - Date sélectionnée:', selected.toISOString());
        setFormData((s) => ({ ...s, date: selected }));
      }
    }
  };
  // ---- FIN FIX
  ...
  useEffect(() => {
    const currentUser = auth.currentUser;
    if (!currentUser) {
      Alert.alert('Authentification requise', 'Vous devez être connecté pour créer un rendez-vous', [
    ...
    try {
      ...
      try {
        ...
        if (realCoaches.length > 0) {
          console.log(
            `✅ CRÉATION RDV - ${realCoaches.length} vrais coaches récupérés, remplacement des données de test`,
          );
  ...
  const addInvitedEmail = async () => {
    ...
    try {
      ...
      if (emailVerification.userData) {
        const userName =
          `${emailVerification.userData.firstName || ''} ${emailVerification.userData.lastName || ''}`.trim() ||
          emailVerification.userData.email;
        Alert.alert('Utilisateur ajouté', `${userName} a été ajouté à la liste des invités`);
      }
    } catch (error) {
  ...
  const removeInvitedEmail = (emailToRemove: string) => {
    const newEmails =
      formData.invitedEmails?.filter((email: string) => email !== emailToRemove) || [];
    ...
    switch (currentStep) {
      ...
      case 3:
        return (
          ...
          (formData.type !== 'group' ||
            (Array.isArray(formData.invitedEmails) && formData.invitedEmails.length > 0))
        );
      default:
        return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!userEmail) {
        Alert.alert('Erreur', 'Impossible de récupérer votre adresse email. Veuillez vous reconnecter.');
        return;
      }
      console.log('👤 CRÉATION RDV - Utilisateur connecté:', {
        userId,
      ...
      Alert.alert('Succès !', `Votre demande de rendez-vous a été créée avec succès.`, [
        {
          text: 'OK',
          onPress: () => {
      ...
      Alert.alert('Erreur', 'Une erreur est survenue lors de la création du rendez-vous. Veuillez réessayer.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderStep1 = () => (
    <View style={styles.stepContent}>
      ...
      <TouchableOpacity
        style={[styles.typeOption, formData.type === 'solo' && styles.typeOptionSelected]}
        onPress={() => {
          console.log('🎯 CRÉATION RDV - Type sélectionné: solo');
          setFormData({ ...formData, type: 'solo', invitedEmails: [] });
        }}
      >
        <Text style={[styles.typeOptionText, formData.type === 'solo' && styles.typeOptionTextSelected]}>
          Rendez-vous Solo
        </Text>
        <Text style={styles.typeOptionDescription}>Séance individuelle avec un coach</Text>
      </TouchableOpacity>

      <TouchableOpacity
        style={[styles.typeOption, formData.type === 'group' && styles.typeOptionSelected]}
        onPress={() => {
      ...
      >
        <Text
          style={[styles.typeOptionText, formData.type === 'group' && styles.typeOptionTextSelected]}
        >
          Rendez-vous Groupe
        </Text>
        <Text style={styles.typeOptionDescription}>Séance collective avec plusieurs participants</Text>
      </TouchableOpacity>
    </View>
  );
      ...
      <View style={styles.inputGroup}>
        ...
        <TouchableOpacity style={styles.dateButton} onPress={openDateTimePicker}>
          <Text style={styles.dateButtonText}>
            ...
            {formData.date.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
          </Text>
        </TouchableOpacity>
        ...
        {showPicker && (
          <DateTimePicker
            ...
            // minimumDate uniquement pertinent pour la date
      ...
      <View style={styles.inputGroup}>
        ...
        <View style={styles.durationContainer}>
          {[30, 45, 60, 90, 120].map((duration) => (
            <TouchableOpacity
              key={duration}
              style={[styles.durationButton, formData.duration === duration && styles.durationButtonSelected]}
              onPress={() => {
        ...
        <Text style={styles.durationInfo}>
          Heure de fin:{' '}
          {new Date(formData.date.getTime() + formData.duration * 60 * 1000).toLocaleTimeString(
            'fr-FR',
        ...
        <View style={styles.inputGroup}>
          ...
          {loadingCoaches ? (
            ...
            <View style={styles.noCoachesContainer}>
              <Text style={styles.noCoachesText}>Aucun coach disponible pour le moment</Text>
        ...
        {formData.type === 'group' && (
          <View style={styles.inputGroup}>
            ...
            {(!formData.invitedEmails || formData.invitedEmails.length === 0) && (
              <Text style={styles.helpText}>Ajoutez au moins un email pour un rendez-vous de groupe</Text>
            )}
          </View>
        )}
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.navigationContainer}>
        ...
        {currentStep < 3 ? (
          <TouchableOpacity
            style={[styles.navButton, styles.nextButton, !validateCurrentStep() && styles.disabledButton]}
            onPress={handleNext}
          ...
          >
            {loading ? <ActivityIndicator size="small" color="white" /> : <Text style={styles.submitButtonText}>Créer le rendez-vous</Text>}
          </TouchableOpacity>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  stepIndicator: {
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingVertical: 20, backgroundColor: 'white', marginBottom: 20,
  },
  stepContainer: { flexDirection: 'row', alignItems: 'center' },
  stepCircle: {
    width: 30, height: 30, borderRadius: 15, backgroundColor: '#e0e0e0', justifyContent: 'center', alignItems: 'center',
  },
  stepCircleActive: { backgroundColor: '#007AFF' },
  stepText: { color: '#666', fontSize: 14, fontWeight: 'bold' },
  stepTextActive: { color: 'white' },
  stepLine: { width: 50, height: 2, backgroundColor: '#e0e0e0', marginHorizontal: 10 },
  stepLineActive: { backgroundColor: '#007AFF' },
  stepContent: { padding: 20 },
  stepTitle: { fontSize: 24, fontWeight: 'bold', marginBottom: 20, textAlign: 'center', color: '#333' },
  typeOption: { backgroundColor: 'white', padding: 20, borderRadius: 10, marginBottom: 15, borderWidth: 2, borderColor: '#e0e0e0' },
  typeOptionSelected: { borderColor: '#007AFF', backgroundColor: '#f0f8ff' },
  typeOptionText: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 5 },
  typeOptionTextSelected: { color: '#007AFF' },
  typeOptionDescription: { fontSize: 14, color: '#666' },
  inputGroup: { marginBottom: 20 },
  label: { fontSize: 16, fontWeight: 'bold', marginBottom: 8, color: '#333' },
  textInput: { backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12, fontSize: 16 },
  textArea: { height: 80, textAlignVertical: 'top' },
  dateButton: { backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12 },
  dateButtonText: { fontSize: 16, color: '#333' },
  coachListFull: { flex: 1, minHeight: 300 },
  coachOption: { backgroundColor: 'white', padding: 15, borderRadius: 8, marginBottom: 10, borderWidth: 2, borderColor: '#e0e0e0' },
  coachOptionSelected: { borderColor: '#007AFF', backgroundColor: '#f0f8ff' },
  coachName: { fontSize: 16, fontWeight: 'bold', color: '#333' },
  coachNameSelected: { color: '#007AFF' },
  coachSpecialty: { fontSize: 14, color: '#666', marginTop: 2 },
  emailInputContainer: { flexDirection: 'row', marginBottom: 10 },
  emailInput: {
    flex: 1, backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12, fontSize: 16, marginRight: 10,
  },
  addEmailButton: { backgroundColor: '#007AFF', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 8, justifyContent: 'center' },
  addEmailButtonText: { color: 'white', fontWeight: 'bold' },
  addEmailButtonDisabled: { backgroundColor: '#999', opacity: 0.6 },
  emailList: { maxHeight: 120 },
  emailItem: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white',
    padding: 12, borderRadius: 8, marginBottom: 5, borderWidth: 1, borderColor: '#e0e0e0',
  },
  emailText: { fontSize: 14, color: '#333' },
  removeEmailButton: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#ff4444', justifyContent: 'center', alignItems: 'center' },
  removeEmailText: { color: 'white', fontSize: 18, fontWeight: 'bold' },
  helpText: { fontSize: 12, color: '#666', fontStyle: 'italic', marginTop: 5 },
  navigationContainer: {
    position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', padding: 20, backgroundColor: 'white', borderTopWidth: 1, borderTopColor: '#e0e0e0',
  },
  navButton: { flex: 1, paddingVertical: 15, borderRadius: 8, alignItems: 'center', marginHorizontal: 5 },
  ...
  noCoachesContainer: {
    alignItems: 'center', padding: 20, backgroundColor: 'white', borderRadius: 8, borderWidth: 1, borderColor: '#e0e0e0',
  },
  noCoachesText: { fontSize: 16, color: '#666', textAlign: 'center', marginBottom: 15 },
  ...
  durationContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: 8 },
  durationButton: {
    backgroundColor: '#f0f0f0', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, marginBottom: 8, borderWidth: 1, borderColor: '#ddd',
  },
  durationButtonSelected: { backgroundColor: '#007AFF', borderColor: '#007AFF' },
  durationButtonText: { color: '#333', fontSize: 14, fontWeight: '500' },
  durationButtonTextSelected: { color: 'white' },
  durationInfo: { fontSize: 12, color: '#666', marginTop: 8, fontStyle: 'italic' },
});
